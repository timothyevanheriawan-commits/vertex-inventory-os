    ...
    if (problemItems.length === 0) {
        return (
            <EmptyState
                ...
                action={
                    <Link href="/dashboard/inventory" className="text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-indigo-600 transition-colors">
                        View Catalog →
                    </Link>
                }
            />
            ...
            <div className="overflow-hidden bg-white rounded-xl border border-slate-200 shadow-sm">
                <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50/50 border-b border-slate-200">
                        <tr>
                            <th className="px-5 py-2.5 text-[9px] font-black text-slate-400 uppercase tracking-[0.15em]">
                                Product Catalog
                            </th>
                            <th className="px-5 py-2.5 text-[9px] font-black text-slate-400 uppercase tracking-[0.15em] text-right">
                                <Tooltip content="30-day moving average of daily units sold.">
                                    <div className="flex items-center justify-end gap-1 cursor-help group">
                                        Burn <Info className="h-2.5 w-2.5 text-slate-300 group-hover:text-indigo-500" />
                                    </div>
                                </Tooltip>
                            </th>
                            <th className="px-5 py-2.5 text-[9px] font-black text-slate-400 uppercase tracking-[0.15em] text-right">
                                <Tooltip content="Current units vs. Reorder Point (ROP) trigger.">
                                    <div className="flex items-center justify-end gap-1 cursor-help group">
                                        Runway <Info className="h-2.5 w-2.5 text-slate-300 group-hover:text-indigo-500" />
                                    </div>
                                </Tooltip>
                            </th>
                            <th className="px-5 py-2.5 text-[9px] font-black text-slate-400 uppercase tracking-[0.15em]">
                            ...
                            <tr key={item.product_id} className="group hover:bg-indigo-50/40 transition-all duration-150">
                                ...
                                <td className="px-5 py-2 text-right align-middle">
                                    <div className={cn(
                                        ...
                                        item.stock_level <= item.reorder_point ? "text-rose-600" : "text-amber-600"
                                    )}>
                                        {item.stock_level} / {item.reorder_point}
                                    </div>
                                    <div className="text-[8px] text-slate-400 font-black uppercase mt-1">
                                        {item.days_remaining}d Left
                                    </div>
                                </td>
                                <td className="px-5 py-2 align-middle">
                                    <div className={cn(
                                        "inline-flex items-center gap-1 px-2 py-0.5 rounded border text-[9px] font-black uppercase",
                                ...
                                <td className="px-5 py-2 text-right align-middle">
                                    <Link href={`/dashboard/inventory/${item.product_id}`} className="opacity-0 group-hover:opacity-100 bg-indigo-600 text-white text-[9px] font-black uppercase px-2.5 py-1 rounded shadow-sm hover:bg-indigo-700 transition-all">
                                        View
                                    </Link>
                                </td>
                            </tr>
