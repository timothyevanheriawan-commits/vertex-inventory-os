    ...
    if (!localSales || localSales.length === 0) {
        return (
            <div className="rounded-xl border border-dashed border-slate-200 p-12 text-center bg-white">
                <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest font-sans">No recent transactions found</p>
            </div>
        )
    }
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            <table className="w-full text-left text-sm border-collapse">
                <thead className="bg-slate-50/50 border-b border-slate-200">
                    <tr>
                        <th className="px-6 py-3 font-black text-[9px] uppercase tracking-[0.2em] text-slate-400">Date</th>
                        <th className="px-6 py-3 font-black text-[9px] uppercase tracking-[0.2em] text-slate-400">Product</th>
                        <th className="px-6 py-3 font-black text-[9px] uppercase tracking-[0.2em] text-slate-400 text-right">Qty</th>
                    ...
                    {localSales.map((sale) => (
                        <tr key={sale.id} className="group hover:bg-slate-50/80 transition-colors animate-in fade-in duration-300">
                            ...
                            <td className="px-6 py-3">
                                ...
                                <div className="text-[10px] font-mono text-slate-400 font-bold uppercase tracking-tighter mt-0.5">{sale.products?.sku}</div>
                            </td>
                            ...
                            <td className="px-6 py-3 text-right">
                                <button
                                    ...
                                    className="opacity-0 group-hover:opacity-100 p-2 text-slate-300 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all cursor-pointer active:scale-90"
